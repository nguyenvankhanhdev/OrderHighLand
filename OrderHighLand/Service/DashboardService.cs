	...
	public class DashboardService
	{
		...
		public async Task<float> getTotalAmountToday()
		{
			...
			try
			{
				...
				{
					var readQuery = "MATCH (o:Order) WHERE o.Date = date() RETURN SUM(o.TotalPrice) as totalAmount";
					var readResult = await transaction.RunAsync(readQuery);
					var total = await readResult.SingleAsync(record =>
					{
						return record[0].As<float>();
		...
		public async Task<int> getOrderToday()
		{
			...
			try
			{
				...
				{
					var readQuery = "MATCH (o:Order) WHERE o.Date = date() RETURN count(o)";
					var readResult = await transaction.RunAsync(readQuery);
					var total = await readResult.SingleAsync(record =>
					{
						return record[0].As<int>();
		...
		public async Task<float> getTotalOrderMonth()
		{
			...
			try
			{
				...
				{
					var readQuery = "MATCH (o:Order) WHERE o.Date >= date() - duration('P1M') RETURN SUM(o.TotalPrice) as totalAmount";
					var readResult = await transaction.RunAsync(readQuery);
					var total = await readResult.SingleAsync(record =>
					{
						return record[0].As<float>();
		...
		public async Task<float> getTotalOrderYear()
		{
			...
			try
			{
				...
				{
					var readQuery = "MATCH (o:Order) WHERE o.Date >= date() - duration('P1Y') RETURN SUM(o.TotalPrice) as totalAmount";
					var readResult = await transaction.RunAsync(readQuery);
					var total = await readResult.SingleAsync(record =>
					{
						return record[0].As<float>();
